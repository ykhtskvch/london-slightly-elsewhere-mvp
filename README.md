...
- `scripts/validate-routes.js` — dependency-free schema and discovery-data
  coverage checks.
- `scripts/validate-site.js` — checks the built pages: one h1, no duplicate
  ids, every local link and image resolves, and the site can be found —
  sitemap and pages agree, nothing listed is `noindex`, every canonical
  matches its sitemap entry, `robots.txt` blocks no search crawler, each
  page's JSON-LD parses, and `llms.txt` lists every walk.
- `sitemap.xml`, `robots.txt`, `llms.txt` — generated by the build from
  `data/site.json` and `data/routes.json`. Do not edit. `llms.txt` is an
  experiment for readers that arrive by language model: the walks, one line
  each, built from the same fields as the pages. Nothing depends on it.
